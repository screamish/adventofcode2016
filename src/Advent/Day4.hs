...
parse' parser i = case parse parser "" i of
...
rooms :: Parser [Room]
rooms = room `sepBy1` eol

room :: Parser Room
room = do
  _ <- sspace
  _cipherName <- fmap (Text.intercalate "-" . fmap Text.pack) $ (try (some letterChar)) `endBy1` char '-'
  _sectorId <- integer
  _ <- char '['
  _checksum <- Text.pack <$> (some letterChar)
  _ <- char ']'
  _ <- sspace
  return Room{..}
  where
...
checksum :: Text -> Text
checksum s =
  let chars = fmap (length &&& head) . group . sort . filter ((/=) '-') $ Text.unpack s
  in Text.pack . take 5 . fmap snd . sortBy (comparing (first Down)) $ chars

isReal :: Room -> Bool
isReal Room{..} =
  (checksum _cipherName) == _checksum
...
partA :: Integer
partA = sectorIdSum input

decrypt :: Text -> Int -> Text
decrypt cipher n = Text.concatMap go cipher
  where go '-' = " "
        go c = Text.pack . (: []) . head . flip drop letters . (+ offset) . normalise $ c
        offset = n `mod` 26
        normalise c = (ord c) - (ord 'a')
        letters = cycle ['a'..'z']
...
partB =
  let decode Room{..} =
          let name = decrypt _cipherName $ fromInteger _sectorId
          in (name, _sectorId)
      findSector = snd . head . filter ((== "northpole object storage") . fst) . fmap decode . filter isReal
  in either id show . fmap findSector . parse' rooms $ input

input :: Text
input = [q|nzydfxpc-rclop-qwzhpc-qtylyntyr-769[oshgk]
